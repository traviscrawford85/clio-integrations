...
class BillUpdateRequestDataDiscount(BaseModel):
    ...
    """ # noqa: E501
    rate: StrictFloat | StrictInt | None = Field(default=None, description="Discount amount for the Bill. This can either be a percentage or monetary value, this is determined by the `discount[type]`.")
    type: StrictStr | None = Field(default=None, description="The type of discount you are applying to your Bill with the `discount[rate]`.")
    note: StrictStr | None = Field(default=None, description="A note for your Bill's discount.")
    __properties: ClassVar[list[str]] = ["rate", "type", "note"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(['percentage', 'money']):
