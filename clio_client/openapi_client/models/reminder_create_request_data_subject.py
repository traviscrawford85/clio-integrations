...
class ReminderCreateRequestDataSubject(BaseModel):
    ...
    """ # noqa: E501
    id: StrictInt = Field(description="The unique identifier for a single CalendarEntry and Task associated with the Reminder. The keyword `null` is not valid for this field.")
    type: StrictStr = Field(description="Model type of the Subject.")
    __properties: ClassVar[List[str]] = ["id", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['CalendarEntry', 'Task']):
