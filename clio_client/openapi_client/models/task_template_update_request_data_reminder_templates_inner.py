...
class TaskTemplateUpdateRequestDataReminderTemplatesInner(BaseModel):
    ...
    notification_type: StrictStr | None = Field(default=None, description="Notification method.")
    id: StrictInt | None = Field(default=None, description="The unique identifier for a single ReminderTemplate associated with the TaskTemplate. The keyword `null` is not valid for this field.")
    destroy: StrictBool | None = Field(default=None, description="The destroy flag. If the flag is set to `true` and the unique identifier of the associated ReminderTemplate is present, the ReminderTemplate is deleted from the TaskTemplate.", alias="_destroy")
    __properties: ClassVar[list[str]] = ["duration_value", "duration_unit", "notification_type", "id", "_destroy"]
    ...
    def notification_type_validate_enum(cls, value):
        ...
        if value not in set(['Email', 'Popup']):
