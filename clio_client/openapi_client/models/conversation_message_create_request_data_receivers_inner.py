...
class ConversationMessageCreateRequestDataReceiversInner(BaseModel):
    ...
    """ # noqa: E501
    id: StrictInt = Field(description="The unique identifier for a single receiver for this ConversationMessage.")
    type: StrictStr = Field(description="The type for a single receiver for this ConversationMessage, could be `Contact` or `User`.")
    __properties: ClassVar[list[str]] = ["id", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Contact', 'User']):
