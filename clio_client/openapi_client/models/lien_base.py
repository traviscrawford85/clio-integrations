...
class LienBase(BaseModel):
    ...
    LienBase
    """ # noqa: E501
    id: StrictInt | None = Field(default=None, description="Unique identifier for the *Lien*")
    amount: StrictFloat | StrictInt | None = Field(default=None, description="The amount for Lien")
    description: StrictStr | None = Field(default=None, description="Lien description")
    etag: StrictStr | None = Field(default=None, description="ETag for the *Lien*")
    lien_type: StrictStr | None = Field(default=None, description="Lien type")
    mark_as_lien: StrictBool | None = Field(default=None, description="Mark item as Lien")
    created_at: datetime | None = Field(default=None, description="The time the *Lien* was created (as a ISO-8601 timestamp)")
    updated_at: datetime | None = Field(default=None, description="The time the *Lien* was last updated (as a ISO-8601 timestamp)")
    __properties: ClassVar[list[str]] = ["id", "amount", "description", "etag", "lien_type", "mark_as_lien", "created_at", "updated_at"]

    @field_validator('lien_type')
    def lien_type_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['general', 'medical_payer', 'medical_provider']):
