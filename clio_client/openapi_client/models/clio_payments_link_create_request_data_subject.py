...
class ClioPaymentsLinkCreateRequestDataSubject(BaseModel):
    ...
    """ # noqa: E501
    id: StrictInt = Field(description="The ID of the record associated with the payment link.")
    type: StrictStr = Field(description="The type of the record associated with the payment link. The type of record determines the behavior of the payment link.  Type of subject object: * 'BankAccount' results in a payment link for a direct payment. * 'Bill' results in a payment link for paying an existing invoice or trust request. * 'Contact' results in a payment link for paying the outstanding invoices of a contact. ")
    __properties: ClassVar[list[str]] = ["id", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['BankAccount', 'Bill', 'Contact']):
